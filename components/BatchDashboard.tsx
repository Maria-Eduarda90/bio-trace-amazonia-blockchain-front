    ...
    if (loading) return (
        <div className="min-h-screen flex items-center justify-center bg-slate-50">
            <div className="text-center">
                ...
                <p className="text-slate-500 font-medium">Rastreando Lote {batchId}...</p>
            </div>
        </div>
    );
    ...
    return (
        <div className="min-h-screen bg-slate-50 pb-20">
            ...
            <main className="max-w-5xl mx-auto px-4 py-8 space-y-8">
                ...
                >
                    <StarIcon size={32} /> Avaliar este Lote
                </button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="md:col-span-1 bg-gradient-to-br from-emerald-900 to-emerald-800 rounded-2xl p-6 text-white shadow-xl flex flex-col justify-between relative overflow-hidden">
                        ...
                        <div>
                            <h2 className="text-emerald-100 text-sm font-semibold uppercase tracking-wider mb-1">Certificado de Autenticidade</h2>
                            <div className="flex items-baseline gap-2">
                                <span className="text-5xl font-bold tracking-tighter">{cert?.gi || 0}</span>
                                <span className="text-emerald-200 font-medium">Pontuação GI</span>
                            </div>
                        </div>

                        <div className="mt-8 flex items-center gap-4">
                            <div className={`w-16 h-16 rounded-full flex items-center justify-center border-4 shadow-inner ${
                                cert?.certificate === 'Gold'
                                    ? 'bg-yellow-400 border-yellow-200 text-yellow-900'
                                    : cert?.certificate === 'Silver'
                            ...
                            <div>
                                <div className="text-2xl font-bold">{cert?.certificate || 'Padrão'}</div>
                                <div className="text-emerald-200 text-sm">Nível de Qualidade</div>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <div className="text-sm text-slate-600 space-y-1">
                                        ...
                                        {event.eventData.rating && (
                                            <div className="flex items-center gap-1.5 text-pink-600 font-semibold">
                                                <Star size={12} fill="currentColor" /> Avaliação: {event.eventData.rating}/10
                                            </div>
                                        )}
